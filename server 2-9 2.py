...
s2.bind(('45.76.119.50', 10002))
s2.listen(20)

def register_1(sock, addr):
	driver2 = webdriver.Chrome()
	def check_course_1 (id,password,num_1,phone_number,driver_2):
		loginUrl = 'https://login.utexas.edu/login/UI/Login' #设定登陆地址
		driver_2.get(loginUrl) #进入登陆界面
		driver_2.find_element_by_name('IDToken1').send_keys(id) #输入用户名
		driver_2.find_element_by_name('IDToken2').send_keys(password) #输入密码
		driver_2.find_element_by_name('Login.Submit').click() #点击登陆
		...
		while(check_status==False): #如没有开启则判断condition为否，并循环刷新
			...
			if ClassStatus=="open": #如课程开启则判断condition为真
				...
				driver_2.get(register_page) #进入选课界面
				driver_2.find_element_by_name('submit').click()
				driver_2.find_element_by_name('s_unique_add').send_keys(num_1)
				driver_2.find_element_by_xpath('//*[@id="ds_request_STADD"]').click()
				driver_2.find_element_by_xpath('//*[@id="regform"]/input[4]').click()
	...
	check_course_1 (list[0],list[1],list[2],list[3],driver2)
	
while True:
	sock1, addr1 = s2.accept()
