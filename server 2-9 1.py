...
s1.bind(('45.76.119.50', 10001))
s1.listen(20)

def register_1(sock, addr):
	driver1 = webdriver.Chrome()
	def check_course_1 (id,password,num_1,phone_number,driver_1):
		...
		while(check_status==False): #如没有开启则判断condition为否，并循环刷新
			...
			if ClassStatus=="open": #如课程开启则判断condition为真
				...
				driver_1.find_element_by_name('s_unique_add').send_keys(num_1)
				driver_1.find_element_by_xpath('//*[@id="ds_request_STADD"]').click()
	...
	check_course_1 (list[0],list[1],list[2],list[3],driver1)
